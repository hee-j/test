...
int8_t BMP384::beginI2C(I2C &i2c_obj, char slave_adr)
{
    // Initialize sensor
    return begin();
}
...
BMP3_INTF_RET_TYPE BMP384::readRegisters(uint8_t regAddress, uint8_t* dataBuffer, uint32_t numBytes, void *nothing)
{
    ...
    i2c.read(address,cmd+1, numBytes);
...
BMP3_INTF_RET_TYPE BMP384::writeRegisters(uint8_t regAddress, const uint8_t* dataBuffer, uint32_t numBytes, void *nothing)
{
    // Make sure the number of bytes is valid
    if(numBytes == 0)
    {
    ...
    char cmd[numBytes+1];
    memset(cmd, 0, numBytes+1);
    cmd[0] = regAddress;
    memcpy(cmd+1, dataBuffer, numBytes);
    i2c.write(address, cmd, numBytes+1);
...
void BMP384::usDelay(uint32_t period, void *nothing)
{
    wait_us(period);
}